...
public class CircleVisualizerView extends BaseVisualizerView {
  ...
  /**
   * set the multiplier to the circle, by default the multiplier is set to 1. you can provide value
   * more than 1 to increase size of the circle visualizer.
   *
    ...
    if (bytes != null) {
      ...
      for (int i = 0; i < 360; i++, angle++) {
        points[i * 4] =
            (float)
                (getWidth() / 2d
                    + Math.abs(bytes[i * 2]) * radiusMultiplier * Math.cos(Math.toRadians(angle)));
        points[i * 4 + 1] =
            (float)
                (getHeight() / 2d
                    + Math.abs(bytes[i * 2]) * radiusMultiplier * Math.sin(Math.toRadians(angle)));
        points[i * 4 + 2] =
            (float)
                (getWidth() / 2d
